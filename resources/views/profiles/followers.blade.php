                ...
                <div class="w-100 d-flex flex-column align-items-center">
                    ...
                    <div class="row">
                        <ul class="navbar-nav justify-content-md-center fw-bold">
                            <li class="nav-item">
                                <a class="nav-link active" style="border-bottom: 2px solid #439089;" aria-current="page" href="#">Followers</a>
                            </li>
                            <li class="nav-item" style="margin-left: 10px;">
                                <a class="nav-link" href="{{ route('show-followings', ['id' => $user->id]) }}">Following</a>
                            </li>
                        </ul>
                    </div>
                </div>
...
@section('content')
    <div class="row" style="margin-top: 70px">
        <div class="col-md-6 px-5" style="margin-top:130px;">
            ...
            <div class="row mt-1">
                <div class="col-md-11">
                    <form action="{{ route('followers-search') }}" method="GET">
            ...
            <div class="row">
                @if (!isset($searchTerm))
                    {{-- Tidak ada pencarian --}}
                    <div class="col-md-8 d-flex flex-column align-items-center justify-content-center text-white w-100" style="margin-top:50px; height:20vh;">
        ...
        <div class="col-md-6" style="margin-top:130px;">
            <h6 class="text-white fw-bold mb-4">List All Followers</h6>
            @foreach ($followers as $item)
                <div class="row mb-3">
                    <div class="col-md-6 d-flex align-items-center">
                        <a href="{{ route('see-profiles', ['user' => $item->follower_id]) }}" class="text-decoration-none">
                            <div class="d-flex align-items-center">
                                <img class="object-fit-cover rounded-circle me-3" src="{{ asset($item->followerUser->profile_pic) }}" alt="profile pic" height="40px" width="40px">
                                <div class="d-flex flex-column">
                                    <p class="fw-bold mb-0 text-white">{{ $item->followerUser->username }}</p>
                                    <p class="text-secondary mb-0" style="font-size: 13px">{{ $item->followerUser->name }}</p>
                                </div>
                            </div>
                        </a>
                    </div>
                    <div class="col-md-2">
                        @if ($item->id != Auth::id())
                            <button class="btn btn-link text-decoration-none fw-bold ms-3 follow-btn"
                                style="{{ Auth::user()->isFollowing($item->followerUser->id) ? 'color: #d03636;' : 'color: #439089;' }}" type="button" data-user-id="{{ $item->followerUser->id }}">
                                {{ Auth::user()->isFollowing($item->followerUser->id) ? 'Unfollow' : 'Follow' }}
                            </button>
                        @endif
                    </div>
                </div>
